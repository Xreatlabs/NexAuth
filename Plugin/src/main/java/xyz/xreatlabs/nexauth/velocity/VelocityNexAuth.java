          ...
          .whenComplete(
              (result, throwable) -> {
                ...
                if (player
                    ...
                    .getServerInfo()
                    .getName()
                    .equals(result.getAttemptedConnection().getServerInfo().getName())) return;
    ...
    if (pluginPresent("protocolize")) {
      ...
      if (maxProtocol == 760) {
        ...
        try {
          ...
          if (build < 172) {
            logger.warn(
                "Detected protocolize, but in order for the integration to work properly, you must"
                    + " be running Velocity build 172 or newer!");
            return null;
          }
        } catch (Exception e) {
      ...
      if (!projector.compatible()) {
        getLogger()
            .warn(
                "Detected protocolize, however, with incompatible version (2.2.2), please upgrade"
                    + " or downgrade.");
        return null;
      }
