...
public class VelocityPlatformHandle implements PlatformHandle<Player, RegisteredServer> {
  private final VelocityNexAuth plugin;

  public VelocityPlatformHandle(VelocityNexAuth plugin) {
    this.plugin = plugin;
  }
                        ...
                        component ->
                            new RuntimeException(
                                "Failed to move player: "
                                    + Component.empty().append(component).content()))
  ...
  public ServerPing ping(RegisteredServer server) {
    try {
      ...
      return players
          .map(value -> new ServerPing(value.getMax() == -1 ? Integer.MAX_VALUE : value.getMax()))
          .orElse(null);
