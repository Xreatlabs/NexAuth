            ...
            if (names.size() == 1) {
              ...
              logger.warn(
                  "Users %s share the same premium UUID %s, contacting mojang to find the owner"
                      .formatted(Arrays.toString(names.toArray()), premiumUUID));
                  ...
                  if (e.getIssue() == PremiumException.Issue.THROTTLED) {
                    ...
                    try {
                      ...
                      throw new RuntimeException(
                          interruptedException); // Probably should not continue when interrupted
                    }
                  } else {
              ...
              if (user == null) {
                logger.warn(
                    "No owner found for the premium UUID %s, omitting".formatted(premiumUUID));
                continue;
              }
              ...
              if (name == null) {
                ...
                logger.info(
                    "Found owner of the premium UUID %s, name %s".formatted(premiumUUID, name));
              }
            } else {
              continue;
            }
                  ...
                  connection2 -> {
                    var ps2 =
                        connection2.prepareStatement(
                            "UPDATE librepremium_data SET premium_uuid=? WHERE last_nickname=?");
