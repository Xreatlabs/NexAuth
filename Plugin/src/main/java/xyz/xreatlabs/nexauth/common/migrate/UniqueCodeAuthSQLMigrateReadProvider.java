        ...
        connection -> {
          ...
          while (rs.next()) {
            try {
              ...
              var password =
                  rs.getString(
                      "password"); // Unfortunately, this godforsaken plugin stores passwords in
              // plain text
              var premium = rs.getBoolean("premium");

              if (password.equals("n"))
                password =
                    null; // The horrible plugin uses "n" as an indicator for null, makes me think
              // what happens when someone uses "n" as a password

              var hashed =
                  password == null ? null : plugin.getDefaultCryptoProvider().createHash(password);
              ...
              if (premium) {
                ...
                try {
                  ...
                  logger.error(
                      "Error while getting premium UUID for " + name + ": " + e.getMessage());
                }
              }
