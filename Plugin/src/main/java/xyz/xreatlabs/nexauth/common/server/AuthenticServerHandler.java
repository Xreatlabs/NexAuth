...
package xyz.xreatlabs.nexauth.common.server;
...
import xyz.xreatlabs.nexauth.common.event.events.AuthenticLimboServerChooseEvent;
import xyz.xreatlabs.nexauth.common.event.events.AuthenticLobbyServerChooseEvent;
...
public class AuthenticServerHandler<P, S> implements ServerHandler<P, S> {
  ...
  public AuthenticServerHandler(AuthenticNexAuth<P, S> plugin) {
    this.plugin = plugin;
            ...
            .build(
                server -> {
                  ...
                  return Optional.ofNullable(
                      plugin.getConfiguration().get(IGNORE_MAX_PLAYERS_FROM_BACKEND_PING)
    ...
    for (S server : plugin.getPlatformHandle().getServers()) {
      plugin
          .getLogger()
          .debug("Server: " + plugin.getPlatformHandle().getServerName(server) + " | " + server);
    }
    ...
    if (user != null && remember && plugin.getConfiguration().get(REMEMBER_LAST_SERVER)) {
      ...
      if (last != null) {
        ...
        if (server != null) {
          ...
          if (ping != null
              && ping.maxPlayers() > plugin.getPlatformHandle().getConnectedPlayers(server)) {
            return server;
          }
        }
      }
