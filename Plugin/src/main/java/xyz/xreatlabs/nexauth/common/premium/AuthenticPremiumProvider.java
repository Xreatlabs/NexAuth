...
public class AuthenticPremiumProvider implements PremiumProvider {
  ...
  public PremiumUser getUserForName(String name) throws PremiumException {
    ...
    if (isCircuitOpen()) {
      throw new PremiumException(
          PremiumException.Issue.THROTTLED, "Premium provider circuit breaker is open");
    }
              ...
              for (int i = 0; i < fetchers.size(); i++) {
                ...
                try {
                  ...
                  if (e.getIssue() == PremiumException.Issue.SERVER_EXCEPTION
                      || e.getIssue() == PremiumException.Issue.THROTTLED) {
                    openCircuit();
                  }
                  ...
                  if (e.getIssue() == PremiumException.Issue.SERVER_EXCEPTION) {
                    plugin
                        .getLogger()
                        .warn(
                            "Got a server exception while fetching premium user. Falling back to an"
                                + " alternative API. Player's information's might not be"
                                + " up-to-date.",
                            e);
                    ...
                    plugin
                        .getLogger()
                        .warn(
                            "Your IP has been rate limited while fetching premium user. Falling"
                                + " back to an alternative API. Player's information's might not be"
                                + " up-to-date.",
                            e);
                  } else {
                    plugin
                        .getLogger()
                        .warn(
                            "Got unexpected exception while fetching premium user. Falling back to"
                                + " an alternative API. Player's information's might not be"
                                + " up-to-date.",
                            e);
                  }
                  ...
                  plugin
                      .getLogger()
                      .debug("Unexpected exception while fetching premium user " + finalName, e);
                  if (firstException == null) {
  ...
  private PremiumUser getUserFromAshcon(String name) throws PremiumException {
    try {
      ...
      var connection =
          (HttpURLConnection)
              new URL("https://api.ashcon.app/mojang/v2/user/" + name).openConnection();
      ...
      switch (connection.getResponseCode()) {
        case 200 -> {
          var data =
              AuthenticNexAuth.GSON.fromJson(
          ...
          return new PremiumUser(
              UUID.fromString(uuid.getAsString()), username, username.equalsIgnoreCase(name));
        }
        case 404 -> {
          return null;
        }
  ...
  private PremiumUser getUserFromPlayerDB(String name) throws PremiumException {
    try {
      ...
      var connection =
          (HttpURLConnection)
              new URL("https://playerdb.co/api/player/minecraft/" + name).openConnection();
      connection.setConnectTimeout(5000);
      connection.setReadTimeout(5000);
  ...
  private PremiumUser getUserFromMinetools(String name) throws PremiumException {
    try {
      plugin.reportMainThread();
      var connection =
          (HttpURLConnection) new URL("https://api.minetools.eu/uuid/" + name).openConnection();
      connection.setConnectTimeout(5000);
      connection.setReadTimeout(5000);
    ...
    try {
      ...
      return switch (connection.getResponseCode()) {
        ...
        case 403 -> {
          ...
          throw new PremiumException(
              PremiumException.Issue.UNDEFINED, GeneralUtil.readInput(connection.getErrorStream()));
        }
        case 500 ->
            throw new PremiumException(
                PremiumException.Issue.SERVER_EXCEPTION,
