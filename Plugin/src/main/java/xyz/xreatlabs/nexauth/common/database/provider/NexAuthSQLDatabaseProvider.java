...
public abstract class NexAuthSQLDatabaseProvider
    extends AuthenticDatabaseProvider<SQLDatabaseConnector> {

  public NexAuthSQLDatabaseProvider(SQLDatabaseConnector connector, AuthenticNexAuth<?, ?> plugin) {
        ...
        connection -> {
          var ps =
              connection.prepareStatement(
                  "SELECT * FROM librepremium_data WHERE LOWER(last_nickname)=LOWER(?)");
  ...
  public User getByPremiumUUID(UUID uuid) {
    ...
    return connector.runQuery(
        connection -> {
          var ps =
              connection.prepareStatement("SELECT * FROM librepremium_data WHERE premium_uuid=?");
  ...
  public void insertUser(User user) {
    ...
    connector.runQuery(
        connection -> {
          var ps =
              connection.prepareStatement(
                  "INSERT INTO librepremium_data(uuid, premium_uuid, hashed_password, salt, algo,"
                      + " last_nickname, joined, last_seen, secret, ip, last_authentication,"
        ...
        connection -> {
          var ps =
              connection.prepareStatement(
                  "INSERT "
                      + getIgnoreSyntax()
                      + " INTO librepremium_data(uuid, premium_uuid, hashed_password, salt, algo,"
                      + " last_nickname, joined, last_seen, secret, ip, last_authentication,"
                      + " last_server, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                      + getIgnoreSuffix());
  ...
  public void updateUser(User user) {
    ...
    connector.runQuery(
        connection -> {
          var ps =
              connection.prepareStatement(
                  "UPDATE librepremium_data SET premium_uuid=?, hashed_password=?, salt=?, algo=?,"
                      + " last_nickname=?, joined=?, last_seen=?, secret=?, ip=?,"
                      + " last_authentication=?, last_server=?, email=? WHERE uuid=?");

          ps.setString(1, user.getPremiumUUID() == null ? null : user.getPremiumUUID().toString());
          ps.setString(
              2, user.getHashedPassword() == null ? null : user.getHashedPassword().hash());
          ps.setString(
              3, user.getHashedPassword() == null ? null : user.getHashedPassword().salt());
          ps.setString(
              4, user.getHashedPassword() == null ? null : user.getHashedPassword().algo());
    ...
    connector.runQuery(
        connection -> {
          ...
          if (!columns.contains("secret"))
            connection
                .prepareStatement(
                    "ALTER TABLE librepremium_data ADD COLUMN secret VARCHAR(255) NULL DEFAULT"
                        + " NULL")
                .executeUpdate();
          if (!columns.contains("ip"))
            connection
                .prepareStatement(
                    "ALTER TABLE librepremium_data ADD COLUMN ip VARCHAR(255) NULL DEFAULT NULL")
                .executeUpdate();
          if (!columns.contains("last_authentication"))
            connection
                .prepareStatement(
                    "ALTER TABLE librepremium_data ADD COLUMN last_authentication TIMESTAMP NULL"
                        + " DEFAULT NULL")
                .executeUpdate();
          if (!columns.contains("last_server")) {
            connection
                .prepareStatement(
                    "ALTER TABLE librepremium_data ADD COLUMN last_server VARCHAR(255) NULL DEFAULT"
                        + " NULL")
                .executeUpdate();
          }
          if (!columns.contains("email")) {
            connection
                .prepareStatement(
                    "ALTER TABLE librepremium_data ADD COLUMN email VARCHAR(255) NULL DEFAULT NULL")
                .executeUpdate();
          }
