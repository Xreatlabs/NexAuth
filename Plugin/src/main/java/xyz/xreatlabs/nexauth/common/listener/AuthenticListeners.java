...
public class AuthenticListeners<Plugin extends AuthenticNexAuth<P, S>, P, S> {
  ...
  protected void onPostLogin(P player, User user) {
    ...
    if (user == null) {
      plugin
          .getLogger()
          .error(
              "Failed to load authentication data for player "
                  + platformHandle.getUsernameForPlayer(player)
                  + " after login; skipping NexAuth tracking for this session.");
      return;
    }

    var sessionTime =
        Duration.ofSeconds(plugin.getConfiguration().get(ConfigurationKeys.SESSION_TIMEOUT));
      ...
      plugin
          ...
          .fire(
              ...
              new AuthenticAuthenticatedEvent<>(
                  user, player, plugin, AuthenticatedEvent.AuthenticationReason.PREMIUM));
      ...
      plugin.delay(
          () ->
              plugin
                  ...
                  .sendMessage(plugin.getMessages().getMessage("info-session-logged-in")),
          500);
      plugin
          .getEventProvider()
          .fire(
              plugin.getEventTypes().authenticated,
              new AuthenticAuthenticatedEvent<>(
                  user, player, plugin, AuthenticatedEvent.AuthenticationReason.SESSION));
    } else {
  ...
  protected PreLoginResult onPreLogin(String username, InetAddress address) {
    if (username.length() > 16 || !NAME_PATTERN.matcher(username).matches()) {
      return new PreLoginResult(
          PreLoginState.DENIED, plugin.getMessages().getMessage("kick-illegal-username"), null);
    }
      ...
      var message =
          switch (e.getIssue()) {
            ...
            default -> {
              plugin
                  .getLogger()
                  .error("Encountered an exception while communicating with the Mojang API!");
              e.printStackTrace();
    ...
    if (mojangData == null) {
      // A user with this name does not exist in the Mojang database. It is impossible for this user
      // to be premium.
      User user;
      try {
      ...
      if (user.getPremiumUUID() != null) {
        // We will have to encrypt, otherwise someone could forcefully disable other user's premium
        // autologin
      ...
      if (user == null) {
        ...
        if (byName != null && !user.equals(byName)) {
          // A user with this name already exists, however, it is not the same user as the premium
          // one.
  ...
  private PreLoginResult handleProfileConflict(User conflicting, User conflicted) {
    return switch (ProfileConflictResolutionStrategy.valueOf(
        plugin.getConfiguration().get(ConfigurationKeys.PROFILE_CONFLICT_RESOLUTION_STRATEGY))) {
      case BLOCK ->
          new PreLoginResult(
              PreLoginState.DENIED,
              plugin
                  .getMessages()
                  .getMessage("kick-name-mismatch", "%nickname%", conflicting.getLastNickname()),
              null);
    ...
    if (user != null) {
      ...
      if (!user.getLastNickname().contentEquals(username)) {
        throw new InvalidCommandArgument(
            plugin
                .getMessages()
                .getMessage("kick-invalid-case-username", "%username%", user.getLastNickname()));
      }
    } else if (generate) {
      ...
      if (username.length() < minLength) {
        throw new InvalidCommandArgument(
            plugin
                .getMessages()
                .getMessage("kick-short-username", "%length%", String.valueOf(minLength)));
      }
      ...
      if (ipLimit > 0) {
        var ipCount =
            plugin
                ...
                .size(); // Ideally, this should be a count query, but I'm too lazy to implement
        // that and the performance impact is negligible.

        if (ipCount >= ipLimit) {
          throw new InvalidCommandArgument(
              plugin.getMessages().getMessage("kick-ip-limit", "%limit%", String.valueOf(ipLimit)));
        }
      }

      var newID = plugin.generateNewUUID(username, premiumUser == null ? null : premiumUser.uuid());
      ...
      if (conflictingUser != null) {
        throw new InvalidCommandArgument(
            plugin
                .getMessages()
                .getMessage(
                    "kick-occupied-username", "%username%", conflictingUser.getLastNickname()));
      }
      ...
      if (premiumUser != null
        ...
        if (!premiumUser.name().contentEquals(username)) {
          throw new InvalidCommandArgument(
              plugin
                  .getMessages()
                  .getMessage("kick-invalid-case-username", "%username%", premiumUser.name()));
        }
        user =
            new AuthenticUser(
                newID,
  ...
  protected BiHolder<Boolean, S> chooseServer(P player, @Nullable String ip, @Nullable User user) {
    ...
    if (fromFloodgate) {
      return new BiHolder<>(
          true, plugin.getServerHandler().chooseLobbyServer(null, player, true, false));
    }
    ...
    if (user == null) {
      plugin
          ...
          .error(
              "Failed to load authentication data for player "
                  + platformHandle.getUsernameForPlayer(player)
                  + " during server selection; falling back to lobby routing.");
      return new BiHolder<>(
          true, plugin.getServerHandler().chooseLobbyServer(null, player, true, false));
    }
