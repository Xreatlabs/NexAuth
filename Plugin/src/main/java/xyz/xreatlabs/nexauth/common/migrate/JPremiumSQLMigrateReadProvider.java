            ...
            try {
              ...
              HashedPassword password =
                  rawPassword == null
                      ? null
                      : switch (split[0]) {
                        case "SHA256" -> new HashedPassword(split[2], split[1], "SHA-256");
                        case "SHA512" -> new HashedPassword(split[2], split[1], "SHA-512");
                        case "BCRYPT" ->
                            CryptoUtil.convertFromBCryptRaw(rawPassword.replace("BCRYPT", "$2a"));
                        default -> {
                          logger.error(
                              "User %s has invalid algorithm %s, omitting"
                                  .formatted(lastNickname, split[0]));
                          yield null;
                        }
                      };
