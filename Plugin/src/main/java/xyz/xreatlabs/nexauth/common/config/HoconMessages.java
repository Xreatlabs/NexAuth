...
public class HoconMessages implements Messages {
  ...
  @Override
  public void reload(NexAuthPlugin<?, ?> plugin)
...
""",
            ...
            (key, value) -> {
              ...
              if (value.childrenMap().isEmpty()) {
                ...
                messages.put(
                    prefix + str,
                    Component.empty()
                        .append(SERIALIZER.deserialize(LegacyMessage.fromLegacy(string, "&"))));
              } else {
  ...
  /**
   * Returns the raw ConfigurateHelper for the messages configuration. Used by ConfigSnapshot to
   * capture message values before reload.
   */
