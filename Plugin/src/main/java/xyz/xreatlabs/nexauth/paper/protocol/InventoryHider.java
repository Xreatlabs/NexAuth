...
public class InventoryHider {
  ...
  public void sendEmptyInventory(Player player) {
    try {
      // Create empty items for player inventory (46 slots: 36 inventory + 4 armor + 1 offhand + 5
      // crafting)
      ...
      plugin
          .getLogger()
          .warn(
              "Failed to send empty inventory to player "
                  + player.getName()
                  + ": "
                  + e.getMessage());
    }
  ...
  public void sendEmptySlot(Player player, int slot) {
    try {
      ...
      plugin
          .getLogger()
          .warn("Failed to send empty slot to player " + player.getName() + ": " + e.getMessage());
    }
  }
