...
/**
 * Packet listener to hide player inventory for unauthenticated players Only works on Paper/Spigot
 * servers
 */
  ...
  public void onPacketReceive(PacketReceiveEvent event) {
    ...
    if (isInventoryInteractionPacket(event.getPacketType())) {
      ...
      plugin
          .getLogger()
          .debug(
              "Blocked inventory interaction packet for unauthenticated player: "
                  + player.getName());
    }
  }
  ...
  private void handleWindowItemsPacket(PacketSendEvent event) {
    try {
      ...
      plugin
          .getLogger()
          .debug(
              "Hid window items for unauthenticated player: "
    ...
    try {
      ...
      if (setSlot.getWindowId() == 0) {
        ...
        plugin
            .getLogger()
            .debug(
                "Hid inventory slot for unauthenticated player: "
                    + ((Player) event.getPlayer()).getName());
      }
            ...
            () -> {
              try {
                ...
                plugin
                    .getLogger()
                    .warn(
                        "Failed to hide inventory for player "
                            + player.getName()
                            + ": "
                            + e.getMessage());
              }
            ...
            () -> {
              try {
                ...
                plugin
                    .getLogger()
                    .warn(
                        "Failed to reveal inventory for player "
                            + player.getName()
                            + ": "
                            + e.getMessage());
              }
