...
public class PaperNexAuth extends AuthenticNexAuth<Player, World> {
  ...
  protected void enable() {
    ...
    if (Bukkit.getOnlineMode()) {
      getLogger()
          .error(
              "!!!The server is running in online mode! NexAuth won't start unless you set it to"
                  + " false!!!");
      disable();
      return;
    }
    ...
    if (Bukkit.spigot().getPaperConfig().getBoolean("settings.velocity-support.enabled")) {
      ...
      getLogger()
          .error(
              "If you want to use NexAuth under a proxy, place it on the proxy and remove it from"
                  + " the server.");
      disable();
      return;
    }
  ...
  public CancellableTask delay(Runnable runnable, long delayInMillis) {
    var task =
        Bukkit.getScheduler().runTaskLaterAsynchronously(bootstrap, runnable, delayInMillis / 50);
    return task::cancel;
  }
