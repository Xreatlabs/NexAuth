...
public class ClientConnection extends ChannelInboundHandlerAdapter {
  ...
  public ClientConnection(
      Channel channel, LimboServer server, PacketDecoder decoder, PacketEncoder encoder) {
    this.server = server;
  ...
  public void spawnPlayer() {
    ...
    Runnable sendPlayPackets =
        () -> {
          ...
          if (server.getPacketSnapshots().getPacketBossBar() != null
              && clientVersion.moreOrEqual(Version.V1_9))
          ...
          if (server.getPacketSnapshots().getPacketTitleTitle() != null
              && clientVersion.moreOrEqual(Version.V1_8)) writeTitle();

          if (server.getPacketSnapshots().getPacketHeaderAndFooter() != null
              && clientVersion.moreOrEqual(Version.V1_8))
  ...
  boolean checkVelocityKeyIntegrity(ByteMessage buf) {
    ...
    try {
      ...
      mac.init(
          new SecretKeySpec(server.getConfig().getInfoForwarding().getSecretKey(), "HmacSHA256"));
    ...
    if (version != 1)
      throw new IllegalStateException(
          "Unsupported forwarding version " + version + ", wanted " + '\001');
    return true;
  }
}
