...
public class ChannelTrafficHandler extends ChannelInboundHandlerAdapter {
  ...
  public ChannelTrafficHandler(int maxPacketSize, double interval, double maxPacketRate) {
    ...
    this.packetBucket =
        (interval > 0.0 && maxPacketRate > 0.0) ? new PacketBucket(interval * 1000.0, 150) : null;
  }
  ...
  public void channelRead(@NotNull ChannelHandlerContext ctx, @NotNull Object msg)
    ...
    if (msg instanceof ByteBuf) {
      ...
      if (maxPacketSize > 0 && bytes > maxPacketSize) {
        closeConnection(
            ctx,
            "Closed %s due to large packet size (%d bytes)",
      ...
      if (packetBucket != null) {
        ...
        if (packetBucket.getCurrentPacketRate() > maxPacketRate) {
          closeConnection(
              ctx,
              "Closed %s due to many packets sent (%d in the last %.1f seconds)",
              ctx.channel().remoteAddress(),
              packetBucket.sum,
              (packetBucket.intervalTime / 1000.0));
          return;
        }
      }
    }
