...
public final class LimboServer {
  ...
  public LimboServer(
      LimboConfig config, CommandHandler<Command> commandHandler, ClassLoader classLoader) {
    this.config = config;
  ...
  public void start() throws Exception {
    ...
    keepAliveTask =
        workerGroup.scheduleAtFixedRate(this::broadcastKeepAlive, 0L, 5L, TimeUnit.SECONDS);
  ...
  private void startBootstrap() {
    ...
    if (config.isUseEpoll() && Epoll.isAvailable()) {
      bossGroup =
          new MultiThreadIoEventLoopGroup(config.getBossGroupSize(), EpollIoHandler.newFactory());
      workerGroup =
          new MultiThreadIoEventLoopGroup(config.getWorkerGroupSize(), EpollIoHandler.newFactory());
      channelFactory = EpollServerSocketChannel::new;
      Log.debug("Using Epoll transport type");
    } else {
      bossGroup =
          new MultiThreadIoEventLoopGroup(config.getBossGroupSize(), NioIoHandler.newFactory());
      workerGroup =
          new MultiThreadIoEventLoopGroup(config.getWorkerGroupSize(), NioIoHandler.newFactory());
